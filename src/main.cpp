        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    }//END lane determination

                    //Create a list of widely spaced <xy> points evenly spaced at 30m, will later tuned by spline
                    vector<double> ptsx;
                    vector<double> ptsy;
                    ...
                    //Frenet implementation. add 30m evenlt spaced points ahead of the starting ref
                    ...
                    //define stragety about how to break up spline points
                    double target_x = 30.0;//herozion
                    double target_y = s(target_x);//spline return the corrsponding y
                    double target_dist = sqrt((target_x)*(target_x)+(target_y)*(target_y));

                    double x_add_on = 0;

                    //fill up the rest of path planner after filling it the previous points
